...
- HAL MCP command namespace (`hal mcp`) supports two transports:
    ...
    - `get_vault_pki_status` is a read-only status tool that runs `hal vault pki` (no flags = status mode) and returns the TEXT execution output covering the Root/Intermediate CA engines (`pki-root` + `pki-int`), the `hal-role` issuing role, and the optional demo layers: cert-manager (`--k8s`: ClusterIssuer `vault-pki-issuer` + Certificate `hal-web-pki-cert` on the nginx pod at `https://pki.localhost:8089`) and ACME/Caddy (`--acme`: role `acme-demo`, 5m TTL auto-renewed at ~1/3 lifetime via Caddy at `https://acme.localhost:8090`). `recommended_commands` lead with `hal vault pki`, then the `enable`/`--acme`/`--k8s` variants. Private keys never leave Vault.
...
- `hal vault oidc` deploys Authentik as a shared IdP and wires Vault OIDC auth. See `docs/scim-idp-spec.md` for full architecture.
    ...
    - **Authentik containers**: `hal-authentik-pg` (postgres:17-alpine), `hal-authentik-server`, `hal-authentik-worker` — all on `hal-net`. No static IPs (Docker DNS). No Docker socket mounted.
