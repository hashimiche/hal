...
- `hal tf saml` deploys Authentik as a shared IdP and wires TFE SAML SSO. See `docs/scim-idp-spec.md` for full architecture.
    ...
    - **SCIM (`--scim`, pending)**: will create TFE org-scoped SCIM token via `POST /api/v2/organizations/:org/scim-tokens`, configure Authentik outbound SCIM provider `tfe-scim-provider` with `verify_ssl: false` (TFE self-signed cert), assign as backchannel on `tfe-saml` application, run `syncTFESCIMObjects` for initial consistency.
