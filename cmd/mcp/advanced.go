...
func baseActionSpecs() []actionSpec {
	return []actionSpec{
		...
		{
			ID:           "vault_mariadb_enable",
			Command:      []string{"vault", "mariadb", "--enable"},
			Aliases:      []string{"enable vault mariadb", "start vault mariadb"},
			Examples:     []string{"hal vault mariadb --enable", "hal --dry-run vault mariadb --enable"},
			Dependencies: []string{"docker_or_podman", "vault"},
			Resources:    []string{"hal-vault-mariadb", "vault database secrets engine"},
			SideEffects:  "creates MariaDB and configures Vault dynamic credentials",
...
func validateCommand(proposed string) map[string]interface{} {
	...
	validProducts := map[string][]string{
		...
		"vault":     {"deploy", "status", "destroy", "audit", "jwt", "k8s", "ldap", "mariadb", "oidc"},
...
func buildStructuredStatus() (map[string]interface{}, error) {
	...
	products := []map[string]interface{}{
		...
		buildProductState(engine, "vault", []string{"hal-vault"}, map[string]string{"audit": resolveVaultAuditFeature(engine), "k8s": boolState(checkContainer(engine, "kind-control-plane")), "jwt": boolState(checkContainer(engine, "hal-gitlab")), "ldap": boolState(checkContainer(engine, "hal-openldap")), "mariadb": boolState(checkContainer(engine, "hal-mariadb")), "oidc": boolState(checkContainer(engine, "hal-keycloak"))}, "http://vault.localhost:8200"),
