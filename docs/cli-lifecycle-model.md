...
| `hal terraform` (alias `hal tf`) | `create`, `delete`, `status`, `obs`, `agent`, `api-workflow` (alias `api`), `workspace` | Product lifecycle is command-based (`create`/`delete`) with `--update` on `create`. `create --vault-enabled` wires external `hal-vault` Dynamic Provider Credentials for the primary TFE issuer using a JWT mount and global `TFC_VAULT_*` variable set. With `--target both`, primary wiring completes and twin deployment continues; `--target twin --vault-enabled` is rejected because twin issuer wiring is not implemented. Twin lifecycle otherwise remains target-based via `--target primary|twin|both`. Terraform observability artifacts are managed under `hal terraform obs <create|update|delete|status>`. |
